  ...
  UserWithEntities: {
    async profile(queries, ctx) {
      ...
      return queries.map(({ obj, params }) =>
        db.profiles.findOne({ key: 'userId', equals: obj.id })
      );
    },
    ...
    async posts(queries, ctx) {
      ...
      return queries.map(async ({ obj, params }) => {
        const posts = await db.posts.findMany({ key: 'userId', equals: obj.id });

        return posts.map(post =>
          db.posts.findOne({ key: 'id', equals: post.id })
        );
      });
    },
    ...
    async memberType(queries, ctx) {
      ...
      return queries.map(async ({ obj, params }) => {
        const profile = await db.profiles.findOne({ key: 'userId', equals: obj.id });
  ...
  UsersSubscriberProfiles: {
    async subscribedProfile(queries, ctx) {
      ...
      return queries.map(async ({ obj, params }) => {
        ...
        return subscribedToUserIds.map(async (id: string) =>
    ...
    async subscriberPosts(queries, ctx) {
      ...
      return queries.map(async ({ obj, params }) => {
        const users: UserEntity[] =
          await db.users.findMany({ key: 'subscribedToUserIds', inArray: obj.id });
      ...
      return queries.map(async ({ obj, params }) => {
        ...
        if (user) {
          const followers: UserEntity[] =
            await db.users.findMany({ key: 'subscribedToUserIds', inArray: user.id });

          return Promise.all(followers.map(async user => {
            const nestedUser =
              await db.users.findOne({ key: 'id', equals: user.id });
    ...
    async userSubscribedTo(queries, ctx) {
      ...
      return queries.map(async ({ obj, params }) => {
        ...
        if (user) {
          ...
          return Promise.all(subscribedToUserIds.map(async id => {
            const nestedUser =
